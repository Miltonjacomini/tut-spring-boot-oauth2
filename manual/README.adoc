...
`@ConfigurationProperties`. That means two things, we need set @Primary in 
the `facebookResource()` to say to `UserInfoTokenServices` who is the primary 
`@Bean` to be inject.
...
And we can change `application.yml` to a slightly new format, where the prefix for
